  ...
  const kpis = [
    {
      ...
      d: "منذ انطلاق مساحتك",
      icon: CircleCheck,
      tone: "bg-jade/12 text-jade",
    },
    {
      k: "قيد التنفيذ",
      v: String(running.length),
      d: "فريقك يعمل الآن",
      icon: PlayCircle,
          ...
          >
            ...
            <div className="relative grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2">
              <div className="min-w-0">
                <p className="truncate text-[0.72rem] font-semibold text-muted-foreground sm:text-sm">
                  {k.k}
                </p>
                <p className="mt-1.5 font-display text-[1.7rem] font-black tabular-nums leading-none sm:text-4xl">
                  {k.v}
                </p>
              </div>
            ...
            <ul className="mt-5 space-y-3">
              {list.slice(0, 6).map((t) => {
                ...
                return (
                  <li
                    key={t.id}
                    className="rounded-xl border border-border/70 p-4 transition-colors hover:bg-secondary/35"
                  >
                    ...
                    {t.detail ? (
                      <p className="mt-1 line-clamp-2 break-words text-sm leading-relaxed text-ink-soft">
                        {t.detail}
                      </p>
                    ) : null}
                  </li>
