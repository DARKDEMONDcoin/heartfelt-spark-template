...
import { ArrowLeft, BarChart3, BrainCircuit, CalendarCheck2, Check, CheckCircle2, Code2, Instagram, Loader2, LockKeyhole, Megaphone, MessageCircle, MessageSquareText, SearchCheck, Send, ShieldCheck, ShoppingBag, Sparkles, Workflow } from "lucide-react";
...
const sampleOutput = {
  ...
  brief: "صورة الطبق من زاوية ٤٥°، إضاءة دافئة، النص على الثلث العلوي، السعر بخط عريض قرميدي، مقاسات ١٠٨٠×١٣٥٠ و١٠٨٠×١٩٢٠.",
} as const;

/** الفرق بين سهل ومحادثة ذكاء اصطناعي عامة. */
const versus = [
  { q: "الذاكرة", chat: "تشرح مشروعك من جديد كل مرة", sahl: "يعرف نشاطك ونبرتك وعروضك ويبني عليها" },
  ...
  { q: "الاعتماد", chat: "لا توجد مرحلة مراجعة", sahl: "كل إجراء يقف في صفحة الموافقات حتى تعتمده" },
...
const guards = [
  { icon: LockKeyhole, t: "صلاحيات دنيا قابلة للسحب", d: "كل ربط يبدأ بأقل صلاحية ممكنة، وتسحبها من صفحة التكاملات في ثانية." },
  { icon: CheckCircle2, t: "لا إجراء بدون اعتمادك", d: "النشر والردود والميزانيات كلها تقف عند صفحة الموافقات أولًا." },
  { icon: ShieldCheck, t: "لا تدريب على بياناتك", d: "محتوى مشروعك لا يُستخدم لتدريب نماذج عامة، وتُحذف بياناتك عند الطلب." },
  { icon: Code2, t: "سجل تدقيق لكل خطوة", d: "كل إجراء مسجَّل باسمه ووقته والحساب الذي نُفّذ عليه." },
] as const;
...
export function EditorialHomepage() {
  ...
  return <div className="sahl-white-home" dir="rtl">
    ...
    <section className="sahl-section sahl-sample sahl-dark sahl-scene"><div className="sahl-dark-veil" aria-hidden="true" /><div className="sahl-sample-stream" aria-hidden="true"><span /><span /><span /><span /></div><div className="sahl-shell"><Reveal><header className="sahl-section-head is-split"><div><span>مخرج حقيقي · بلا تجميل</span><h2>هذا ما يصلك فعلًا.<br /><em>منسوخ كما يُسلَّم.</em></h2></div><p>طلب واحد بالعامية، والمخرج جاهز للاعتماد: منشور بلهجة جمهورك، نص إعلان مموّل، وبريف تصميم لدانة — دون تعديل من عندنا.</p></header></Reveal><Reveal className="sahl-sample-board"><div className="sahl-sample-ask"><small>طلبك</small><p>{sampleOutput.prompt}</p></div><div className="sahl-sample-grid"><article className="is-post"><header><Instagram aria-hidden="true" /><b>منشور إنستجرام</b><span>لهجة خليجية</span></header><pre>{sampleOutput.post}</pre></article><article className="is-ad"><header><Megaphone aria-hidden="true" /><b>نص إعلان مموّل</b><span>سِراج</span></header><pre>{sampleOutput.ad}</pre></article><article className="is-brief"><header><Sparkles aria-hidden="true" /><b>بريف التصميم</b><span>لدانة</span></header><pre>{sampleOutput.brief}</pre></article></div><p className="sahl-sample-note"><CheckCircle2 aria-hidden="true" /> الأرقام والعروض في المثال من طلب المستخدم نفسه — الفريق لا يخترع سعرًا ولا وعدًا من عنده.</p></Reveal></div></section>

    <section className="sahl-section sahl-capabilities"><div className="sahl-shell"><Reveal><header className="sahl-section-head"><span>ستة تخصصات بسياق واحد</span><h2>كل موظف ينجز دوره.<br /><em>وكل نتيجة تسلّم التالية.</em></h2><p>سِراج يبدأ الحملة، دانة تجهز صورتها، نور توسع قصتها، سالم يحول الاهتمام إلى فرصة، أمَل ترتب الوقت، وآدم يقرأ ما حدث.</p></header></Reveal><div className="sahl-cap-grid">{capabilities.map((item) => <Reveal key={item.kicker} className={`sahl-cap-card is-${item.span} is-${item.tone}`}><div><item.icon /><span>{item.kicker}</span><h3>{item.title}</h3><p>{item.body}</p>{item.kicker.startsWith("سِراج") && <aside className="sahl-coming-ads"><Megaphone aria-hidden="true" /><div><span>تخصص إضافي</span><strong>سِراج يبني حملاتك المموّلة</strong><p>يحسب هامشك وسقف تكلفة التحويل، يضع هيكل الحساب ومصفوفة خمس زوايا كرييتف وخطة القياس. الإطلاق المباشر على المنصات يتم بعد ربطها وبموافقتك.</p></div></aside>}<Link to="/features">شاهد مهامه <ArrowLeft /></Link></div>{item.span === "wide" ? <ProductFrame src={item.image} mobileSrc={item.mobileImage} alt={`واجهة ${item.kicker} داخل سهل`} /> : null}</Reveal>)}</div></div></section>

    <section className="sahl-stats sahl-scene"><div className="sahl-stats-wave" aria-hidden="true"><svg viewBox="0 0 1400 520" preserveAspectRatio="none"><defs><linearGradient id="sahlStatsWave" x1="0%" y1="100%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--home-terracotta)" /><stop offset="48%" stopColor="var(--home-gold)" /><stop offset="100%" stopColor="var(--home-teal)" /></linearGradient></defs><g className="sahl-stats-wave-lines"><path d="M-80 388 C 190 310 338 435 572 342 S 918 126 1510 184" /><path d="M-80 424 C 204 342 358 466 590 374 S 940 158 1510 216" /><path d="M-80 460 C 220 380 378 498 616 410 S 968 192 1510 250" /><path d="M-80 352 C 176 278 322 398 548 308 S 886 96 1510 150" /></g><path className="sahl-stats-wave-light" d="M-80 388 C 190 310 338 435 572 342 S 918 126 1510 184" /></svg></div><div className="sahl-shell"><Reveal><header><span>أرقام من داخل المنصة</span><h2><strong>ما تحصل عليه فعليًا</strong><em> من أول يوم في حسابك.</em></h2></header></Reveal><div className="sahl-stat-grid">{stats.map((item, index) => <Reveal key={item.label} className={`sahl-stat is-${item.tone}`}><small>٠{index + 1}</small><strong>{item.value}</strong><span>{item.label}</span></Reveal>)}</div></div></section>

    <section className="sahl-section sahl-demo-section"><div className="sahl-shell"><Reveal><header className="sahl-section-head is-split"><div><span>جرّب تسليم المهمة</span><h2>قل ما تريد مرة.<br /><em>واستلم خطة الفريق.</em></h2></div><p>طلب واحد من عندك، يتحول إلى ست مهام موزعة على سِراج ودانة ونور وأمَل وسالم وآدم.</p></header></Reveal><Reveal><figure className="sahl-plan-flow"><picture><source media="(max-width: 720px)" srcSet={planFlowTall} /><img src={planFlowWide} alt="طلب واحد يتفرع إلى ست مهام ملونة يتسلمها موظفو سهل" loading="lazy" width={1536} height={864} /></picture><figcaption>من طلب واحد إلى خطة موزعة على الفريق</figcaption></figure></Reveal></div></section>

    <section className="sahl-section sahl-versus sahl-dark sahl-scene"><div className="sahl-dark-veil" aria-hidden="true" /><div className="sahl-versus-beam" aria-hidden="true" /><div className="sahl-shell"><Reveal><header className="sahl-section-head"><span>السؤال الأول دائمًا</span><h2>ولماذا لا أستخدم محادثة ذكاء اصطناعي عادية؟<br /><em>لأن المحادثة تكتب، وسهل يشتغل.</em></h2></header></Reveal><Reveal className="sahl-versus-table"><div className="sahl-versus-head"><span>الفرق</span><span>محادثة عامة</span><span><LogoMark size={18} /> سهل</span></div>{versus.map((row) => <div key={row.q} className="sahl-versus-row"><span>{row.q}</span><span className="is-plain">{row.chat}</span><span className="is-sahl"><Check aria-hidden="true" />{row.sahl}</span></div>)}</Reveal></div></section>
    ...
    <section className="sahl-section sahl-infrastructure sahl-scene"><div className="sahl-shell"><Reveal><header className="sahl-section-head is-split"><div><span>حساباتك وقواعدك</span><h2>الفريق يصل لأدواتك.<br /><em>والقرار يبقى عندك.</em></h2></div><p>سِراج ينشر، سالم يتابع، نور تقرأ البحث، وآدم يجمع الأداء. لا يخرج إجراء حساس قبل قاعدة الاعتماد التي تحددها.</p></header></Reveal><Reveal className="sahl-system-board"><ToolConnections /><div className="sahl-system-points"><span><Workflow /><b>سياق ينتقل بين الستة</b></span><span><LockKeyhole /><b>موافقة قبل التنفيذ</b></span><span><Code2 /><b>سجل واضح لكل خطوة</b></span></div><div className="sahl-system-horizon" aria-hidden="true"><svg viewBox="0 0 1440 440" preserveAspectRatio="none"><defs><linearGradient id="sahlSystemHorizonLine" x1="100%" y1="72%" x2="0%" y2="28%"><stop offset="0%" stopColor="var(--home-terracotta)" /><stop offset="42%" stopColor="var(--home-gold)" stopOpacity=".46" /><stop offset="100%" stopColor="var(--home-teal)" /></linearGradient><linearGradient id="sahlSystemHorizonSignal" x1="100%" y1="70%" x2="0%" y2="30%"><stop offset="0%" stopColor="var(--home-gold)" stopOpacity="0" /><stop offset="45%" stopColor="var(--home-gold)" /><stop offset="100%" stopColor="var(--home-bg)" stopOpacity="0" /></linearGradient></defs><g className="sahl-horizon-lines">{statHorizonPaths.map((path, index) => <path key={path} d={path} style={{ "--line-opacity": 0.52 - index * 0.009 } as CSSProperties} />)}</g><g className="sahl-horizon-signals">{statHorizonSignals.map((pathIndex, index) => <path key={pathIndex} d={statHorizonPaths[pathIndex]} style={{ "--signal-delay": `${index * -3.7}s` } as CSSProperties} />)}</g><g className="sahl-horizon-nodes">{statHorizonNodes.map((node) => <circle key={`${node.cx}-${node.cy}`} cx={node.cx} cy={node.cy} r={node.r} style={{ "--node-delay": node.delay } as CSSProperties} />)}</g></svg></div></Reveal></div></section>

    <section className="sahl-section sahl-proof sahl-dark sahl-scene"><div className="sahl-dark-veil" aria-hidden="true" /><div className="sahl-proof-ledger" aria-hidden="true"><i /><i /><i /><i /><i /><i /></div><div className="sahl-shell"><Reveal><header className="sahl-section-head"><span>إثبات بدل الوعد</span><h2>لا نعرض شهادات مجهولة.<br /><em>نعرض ما يمكنك التحقق منه.</em></h2><p>كل رقم في هذه الصفحة إما موجود داخل المنصة، أو معيار سوق معلن مصدره. ولا نضع اسم عميل لم يوافق على ذكره.</p></header></Reveal><Reveal className="sahl-proof-visual"><figure><img src={proofVisual} alt="طبقات من السجلات والموافقات المتسلسلة داخل سهل" loading="lazy" width={1280} height={960} /><span className="sahl-visual-sweep" aria-hidden="true" /><span className="sahl-visual-ring" aria-hidden="true" /></figure><figcaption>كل مخرج له بطاقة، ولكل بطاقة أثر مكتوب: من طلبها، ومن اعتمدها، ومتى.</figcaption></Reveal><div className="sahl-proof-grid">{proofs.map((item) => <Reveal key={item.title}><article><span>{item.kicker}</span><h3>{item.title}</h3><p>{item.body}</p></article></Reveal>)}</div></div></section>

    <section className="sahl-section sahl-ads sahl-scene"><div className="sahl-shell"><Reveal><header className="sahl-section-head is-split"><div><span>سِراج · الإعلانات المموّلة</span><h2>حملة تُبنى من أرقام مشروعك.<br /><em>لا من تخمين.</em></h2></div><p>سِراج يحسب هامشك وقيمة عميلك أولًا، ثم يبني هيكل الحساب ومصفوفة الكرييتف وخطة القياس ليوم واحد حتى اليوم الرابع عشر — ولا تُعتمد ميزانية ولا تُطلق حملة قبل موافقتك.</p></header></Reveal><Reveal className="sahl-ads-visual"><figure><img src={adsVisual} alt="قمع إعلاني ومنحنى أداء ودوائر استهداف بألوان سهل" loading="lazy" width={1280} height={960} /><span className="sahl-visual-orbit" aria-hidden="true"><i /><i /><i /></span><span className="sahl-visual-sweep" aria-hidden="true" /></figure><div><span>منهج سِراج</span><h3>من الوحدات الاقتصادية إلى الكرييتف إلى القياس.</h3><p>قبل أي ريال إنفاق: نعرف هامشك، وقيمة عميلك، والسقف الذي لا تتجاوزه تكلفة التحويل. ثم يُبنى الحساب والكرييتف والقياس على هذه الأرقام وحدها.</p></div></Reveal><Reveal className="sahl-ads-board"><div className="sahl-ads-metrics">{adsProof.map((item) => <span key={item.k}><small>{item.k}</small><strong>{item.v}</strong><em>{item.note}</em></span>)}</div><ol className="sahl-ads-steps"><li><b>الوحدات الاقتصادية</b><span>هامشك وقيمة عميلك يحددان سقف تكلفة التحويل قبل أي إنفاق.</span></li><li><b>هيكل الحساب</b><span>حملة تحويل واحدة، مجموعات محدودة، وتوزيع ٧٠/٢٠/١٠ على القمع.</span></li><li><b>خمس زوايا كرييتف</b><span>ألم، نتيجة، اعتراض، دليل، مقارنة — بنصوص بلهجة جمهورك وبريف تصميم لدانة.</span></li><li><b>القياس والتشخيص</b><span>بيكسل وأحداث وUTM، ثم شجرة تشخيص تحدد أين يضيع الإنفاق بالضبط.</span></li></ol><p className="sahl-ads-note"><LockKeyhole aria-hidden="true" /> الأرقام أعلاه نطاقات معيارية للسوق تُستخدم للمقارنة، وليست نتائج حساب بعينه. الإطلاق المباشر على منصات الإعلانات يتم عبر الربط الرسمي وبعد اعتمادك.</p></Reveal></div></section>

    <section className="sahl-section sahl-guard sahl-dark sahl-scene"><div className="sahl-dark-veil" aria-hidden="true" /><div className="sahl-guard-radar" aria-hidden="true"><b /><i /><i /><i /></div><div className="sahl-shell"><Reveal><header className="sahl-section-head is-split"><div><span>قبل أن تربط حساباتك</span><h2>نعمل داخل أدواتك.<br /><em>فهذه قواعدنا الأربع.</em></h2></div><p>لا نطلب ثقتك دون شرح ما نفعله ببياناتك وصلاحياتك — والتفاصيل كاملة في <Link to="/security">صفحة الأمان</Link>.</p></header></Reveal><div className="sahl-guard-grid">{guards.map((item) => <Reveal key={item.t}><article><item.icon aria-hidden="true" /><h3>{item.t}</h3><p>{item.d}</p></article></Reveal>)}</div></div></section>

    <section className="sahl-section sahl-pricing"><div className="sahl-shell"><Reveal><header className="sahl-section-head"><span>عدد الموظفين يتبع حجم العمل</span><h2>ابدأ بدور واحد.<br /><em>وأضف الفريق حين تحتاجه.</em></h2><p>لا تدفع مقابل مقاعد لا تعمل. اختر البداية، أو فعّل تعاون الموظفين الستة في خطة النمو. الاشتراك شهري ويُلغى في أي وقت — <Link to="/refunds">سياسة الاسترداد</Link>.</p></header></Reveal><div className="sahl-plan-grid">{plans.map((plan) => <Reveal key={plan.id}><article className={plan.highlight ? "is-featured" : ""}>{plan.highlight && <span className="sahl-plan-tag">الفريق كاملًا</span>}<small>{plan.tag}</small><h3>{plan.name}</h3><div className="sahl-price">{plan.monthly ? <><strong>{plan.monthly.toLocaleString("ar-SA")}</strong><span>ر.س كل شهر</span></> : <strong>تسعير مخصص</strong>}</div><p>{plan.desc}</p><ul>{plan.perks.slice(0, 5).map((perk) => <li key={perk}><Check />{perk}</li>)}</ul><Button asChild variant={plan.highlight ? "default" : "outline"}>{plan.monthly ? <Link to="/auth" search={{ mode: "signup" as const }}>{plan.cta}<ArrowLeft /></Link> : <Link to="/contact">{plan.cta}<ArrowLeft /></Link>}</Button></article></Reveal>)}</div></div></section>
